...
def change_cell_value(object, x, y, z, vox_array, voxel_menu_var, val, add):
    vox_array[x][y][z] = val
    object.data["voxel_array"] = vox_array.tolist() 

def paint_cell_values(object, x, y, z, vox_array, voxel_menu_var, val, add):
    for x_axis in range(len(vox_array)):
        for y_axis in range(len(vox_array[x])):
            for z_axis in range(len(vox_array[x][y])):
...
def probe_cell_value(object, x, y, z, vox_array,  voxel_menu_var, val, add):
...
def probe_set_voxel_smoothness(object, x, y, z, vox_array, voxel_menu_var, val, add):
    voxel_smooth_array = np.array(object.data["voxel_smooth_array"])
...
def execute_at_position(object, offset_position,  voxel_menu_var, val, function_to_execute, add):
    try:
        vox_array = np.array(object.data["voxel_array"])
        ...
        if ((x >= 0) and (x < len(vox_array))):
            if ((y >= 0) and (y < len(vox_array[1]))):
                if ((z >= 0) and (z < len(vox_array[1][1]))):
                    function_to_execute(object, x, y, z, vox_array, voxel_menu_var, val, add)
...
def execute_function_on_clicked_voxel(mouse_x, mouse_y, object, context, add, voxel_menu_var, function_to_execute):
    ...
    if hit:
        ...
        execute_at_position(object, offset_position_obj_space, voxel_menu_var, val, function_to_execute, add)
