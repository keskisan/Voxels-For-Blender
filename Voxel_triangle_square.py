...
def get_vertex_array_offset(vertex_smooth_array, x, y, z):
    ...
    if y > 0: #diagonals for all points
        if vertex_smooth_array[x_half][y - 1][z] > -100:
            y_offset -= vertex_smooth_array[x_half][y - 1][z]/2
            x_offset -= vertex_smooth_array[x_half][y - 1][z]/2
    ...
    if y > 0 and x_half > 0:
        if vertex_smooth_array[x_half - 1][y - 1][z] > -100:
            y_offset -= vertex_smooth_array[x_half - 1][y - 1][z]/2
            x_offset -= vertex_smooth_array[x_half - 1][y - 1][z]/2 
    if y < len(vertex_smooth_array[1]) - 1 and x_half > 0:
        if vertex_smooth_array[x_half - 1][y + 1][z] > -100:
            y_offset += vertex_smooth_array[x_half - 1][y + 1][z]/2
            x_offset += vertex_smooth_array[x_half - 1][y + 1][z]/2   
    ...
    if y % 2 == 0: #even 
        ...
        if x_half < len(vertex_smooth_array) - 1:
            if vertex_smooth_array[x_half + 1][y][z] > -100:
                x_offset += vertex_smooth_array[x_half + 1][y][z]
        if y > 1:
            if vertex_smooth_array[x_half][y - 2][z] > -100:
                y_offset -= vertex_smooth_array[x_half][y - 2][z]
        if y < len(vertex_smooth_array[1]) - 2:
            if vertex_smooth_array[x_half][y + 2][z] > -100:
                y_offset += vertex_smooth_array[x_half][y + 2][z]
...
def add_triangle(vertex_smooth_array, voxel_menu_var, vert_coords, face_vert_indices, a, b, c, x, y, z, uvs, u, v, w,):

    vert_coords.append(get_vertex_array_offset(vertex_smooth_array, a[0] + x, a[1] + y, a[2] + z))
    vert_coords.append(get_vertex_array_offset(vertex_smooth_array, b[0] + x, b[1] + y, b[2] + z))
    vert_coords.append(get_vertex_array_offset(vertex_smooth_array, c[0] + x, c[1] + y, c[2] + z))
...
def add_quad(vertex_smooth_array, voxel_menu_var, vert_coords, face_vert_indices, a, b, c, d, x, y, z, uvs, t, u, v, w):

    vert_coords.append(get_vertex_array_offset(vertex_smooth_array, a[0] + x, a[1] + y, a[2] + z))
    vert_coords.append(get_vertex_array_offset(vertex_smooth_array, b[0] + x, b[1] + y, b[2] + z))
    vert_coords.append(get_vertex_array_offset(vertex_smooth_array, c[0] + x, c[1] + y, c[2] + z))
    vert_coords.append(get_vertex_array_offset(vertex_smooth_array, d[0] + x, d[1] + y, d[2] + z))
...
def calculate_square_smooth_array(smooth_array, vox_array):
     #populate a new array with the average off all the voxel smooth types according to the voxel array number(type)
    vertex_smooth_count = np.zeros((math.floor(len(vox_array)/2 + 1), len(vox_array[1]) + 2, len(vox_array[1][1]) + 1)) #x is only half as 2 triangles to a square
    ...
    for x in range(len(vertex_smooth_count) - 1): 
        for y in range(len(vertex_smooth_count[x]) - 2): #irregularity is due to way triangles alline. Row corner in may not necessarily exist
                        ...
                        if x * 2 + 1 < len(vox_array):    
                            if vox_array[x*2 + 1][y][z] > 0:        #10         
                                if vertex_smooth_array[x][y][z] < -100:
                                   ...
                                   vertex_smooth_array[x][y][z] = vertex_smooth_array[x + 1][y][z] +  smoothfactor
                                vertex_smooth_count[x][y][z] += 1
                                
                                if vertex_smooth_array[x - 1][y + 1][z] < -100:
                                    vertex_smooth_array[x - 1][y + 1][z] = smoothfactor
                                else:
                                   vertex_smooth_array[x - 1][y + 1][z] = vertex_smooth_array[x][y + 1][z] +  smoothfactor
                                vertex_smooth_count[x - 1][y + 1][z] += 1
                                
                                if vertex_smooth_array[x][y + 2][z] < -100:
                                    vertex_smooth_array[x][y + 2][z] = smoothfactor
                                else:
                                   vertex_smooth_array[x][y + 2][z] = vertex_smooth_array[x + 1][y + 1][z] +  smoothfactor
                                ...
                                if vertex_smooth_array[x][y][z + 1] < -100:
                                   ...
                                   vertex_smooth_array[x][y][z + 1] = vertex_smooth_array[x + 1][y][z] +  smoothfactor
                                vertex_smooth_count[x][y][z + 1] += 1
                                
                                if vertex_smooth_array[x - 1][y + 1][z + 1] < -100:
                                    vertex_smooth_array[x - 1][y + 1][z + 1] = smoothfactor
                                else:
                                   vertex_smooth_array[x - 1][y + 1][z + 1] = vertex_smooth_array[x][y + 1][z] +  smoothfactor
                                vertex_smooth_count[x - 1][y + 1][z + 1] += 1
                                
                                if vertex_smooth_array[x][y + 2][z + 1] < -100:
                                    vertex_smooth_array[x][y + 2][z + 1] = smoothfactor
                                else:
                                   vertex_smooth_array[x][y + 2][z + 1] = vertex_smooth_array[x + 1][y + 1][z] +  smoothfactor
                        ...
                        if x * 2 < len(vox_array) and y + 1 < len(vox_array[1]):    
                            if vox_array[x*2][y + 1][z] > 0: #01
                                ...
                                if vertex_smooth_array[x][y + 1][z] < -100:
                                   ...
                                   vertex_smooth_array[x][y + 1][z] = vertex_smooth_array[x][y + 2][z] +  smoothfactor
                                vertex_smooth_count[x][y + 1][z] += 1
                                
                                if vertex_smooth_array[x][y][z] < -100:
                                    vertex_smooth_array[x][y][z] = smoothfactor
                                else:
                                   vertex_smooth_array[x][y][z] = vertex_smooth_array[x + 1][y + 2][z] +  smoothfactor
                                vertex_smooth_count[x][y][z] += 1
                                ...
                                if vertex_smooth_array[x][y - 1][z + 1] < -100:
                                   ...
                                   vertex_smooth_array[x][y - 1][z + 1] = vertex_smooth_array[x][y + 1][z] +  smoothfactor
                                vertex_smooth_count[x][y - 1][z + 1] += 1
                                
                                if vertex_smooth_array[x][y + 1][z + 1] < -100:
                                    vertex_smooth_array[x][y + 1][z + 1] = smoothfactor
                                else:
                                   vertex_smooth_array[x][y + 1][z + 1] = vertex_smooth_array[x][y + 2][z] +  smoothfactor
                                vertex_smooth_count[x][y + 1][z + 1] += 1
                                
                                if vertex_smooth_array[x][y][z + 1] < -100:
                                    vertex_smooth_array[x][y][z + 1] = smoothfactor
                                else:
                                   vertex_smooth_array[x][y][z + 1] = vertex_smooth_array[x + 1][y + 2][z] +  smoothfactor
                                vertex_smooth_count[x][y][z + 1] += 1
                        ...
                        if x * 2 + 1 < len(vox_array) and y + 1 < len(vox_array[1]):    
                            if vox_array[x*2 + 1][y + 1][z] > 0: #11
                                ...
                                if vertex_smooth_array[x][y + 1][z] < -100:
                                   ...
                                   vertex_smooth_array[x][y + 1][z] = vertex_smooth_array[x + 1][y + 2][z] +  smoothfactor
                                vertex_smooth_count[x][y + 1][z] += 1
                                
                                if vertex_smooth_array[x - 1][y][z] < -100:
                                    vertex_smooth_array[x - 1][y][z] = smoothfactor
                                else:
                                   vertex_smooth_array[x - 1][y][z] = vertex_smooth_array[x + 1][y + 2][z] +  smoothfactor
                                vertex_smooth_count[x - 1][y][z] += 1
                                ...
                                if vertex_smooth_array[x][y + 1][z + 1] < -100:
                                   ...
                                   vertex_smooth_array[x][y + 1][z + 1] = vertex_smooth_array[x + 1][y + 2][z] +  smoothfactor
                                vertex_smooth_count[x][y + 1][z + 1] += 1
                                
                                if vertex_smooth_array[x - 1][y][z + 1] < -100:
                                    vertex_smooth_array[x - 1][y][z + 1] = smoothfactor
                                else:
                                   vertex_smooth_array[x - 1][y][z + 1] = vertex_smooth_array[x + 1][y + 2][z] +  smoothfactor
                                vertex_smooth_count[x - 1][y][z + 1] += 1
...
def generate_square_prism_voxels(vertex_smooth_array, vox_array, vert_coords, face_vert_indices, voxel_menu_var, uvs, uv_scale, bufferx, buffery):
        ...
        for y in range(len(vox_array[x])):
            for z in range(len(vox_array[x][y])): 
                if (vox_array[x][y][z] != 0): 
                    ...
                    if x % 2 == 0: #four together form a square like a windmill
                        if y % 2 == 0: #1 x = 0 y = 0
                            if z > 0:
                                if vox_array[x][y][z - 1] == 0:
                                    ...
                                    add_triangle(vertex_smooth_array, voxel_menu_var, vert_coords, face_vert_indices, a, b, c, x, y, z, uvs, u, v, w)   
                            else:
                                a = np.array([0, 0, 0])   
                                b = np.array([1, 1, 0])
                                c = np.array([2, 0, 0])
                            ...
                            if z < len(vox_array[x][y]) - 1:
                                if vox_array[x][y][z + 1] == 0:
                                    ...
                                    v = (mat_x + uv_scale - bufferx, mat_y + uv_scale - buffery) #(1, 1)
                            ...
                            if y < len(vox_array[x]) - 1:
                                if vox_array[x][y + 1][z] == 0:
                                    ...
                                    add_quad(vertex_smooth_array, voxel_menu_var, vert_coords, face_vert_indices, a, b, c, d, x, y, z, uvs, t, u, v, w)   
                            else:
                                a = np.array([1, 1, 0])  #1 
                                b = np.array([0, 0, 0])
                                c = np.array([0, 0, 1])
                            ...
                            if z > 0:
                                if vox_array[x][y][z - 1] == 0:
                                    ...
                                    w = (mat_x + uv_scale - bufferx, mat_y + uv_scale*0.33 - buffery) #(1, 1)
                                    add_triangle(vertex_smooth_array, voxel_menu_var, vert_coords, face_vert_indices, a, b, c, x, y, z, uvs, u, v, w)    
                            else:
                                a = np.array([0, -1, 0])   
                                b = np.array([0, 1, 0])
                            ...
                            if z < len(vox_array[x][y]) - 1:
                                if vox_array[x][y][z + 1] == 0:
                                    ...
                                    w = (mat_x + uv_scale - bufferx, mat_y + uv_scale*0.66 + buffery) #(1, 0)
                                    add_triangle(vertex_smooth_array, voxel_menu_var, vert_coords, face_vert_indices, a, b, c, x, y, z, uvs, u, v, w)    
                            else:
                                a = np.array([1, 0, 1])   
                                b = np.array([0, 1, 1])
                    ...
                    else: #2 x = 0 y = 1 wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww
                        if y % 2 == 0:
                            if z > 0:
                                if vox_array[x][y][z - 1] == 0:
                                    a = np.array([0, 1, 0])   
                            ...
                            if z < len(vox_array[x][y]) - 1:
                                if vox_array[x][y][z + 1] == 0:
                                    ...
                                    u = (mat_x + uv_scale - bufferx, mat_y + uv_scale*0.66 + buffery) #(1, 0)
                                    v = (mat_x + bufferx, mat_y + uv_scale - buffery) #(0, 1)
                                    w = (mat_x + bufferx, mat_y + uv_scale*0.66 + buffery) #(0, 0)
                                    add_triangle(vertex_smooth_array, voxel_menu_var, vert_coords, face_vert_indices, a, b, c, x, y, z, uvs, u, v, w) 
                            else:
                                a = np.array([1, 0, 1])   
                                b = np.array([1, 2, 1])
                            ...
                            if x < len(vox_array) - 1 and y < len(vox_array[x]) - 1:
                                if vox_array[x + 1][y + 1][z] == 0:
                                    a = np.array([1, 0, 0]) #2    
                                    b = np.array([1, 2, 0])
                                    c = np.array([1, 2, 1])
                            ...
                            if x > 0: #wwwwwwwwwwwwwwwwwwwwwwwww
                                if vox_array[x - 1][y][z] == 0:
                                    a = np.array([0, 1, 0]) #3  
                                    b = np.array([1, 0, 0])
                                    c = np.array([1, 0, 1])
                        ...
                        else: # 4 x = 1 y = 1 wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww
                            if z > 0:
                                if vox_array[x][y][z - 1] == 0:
                                    a = np.array([0, 0, 0])   
                                    b = np.array([-1, 1, 0])
                            ...
                            if x > 0 and y < len(vox_array[x]) - 1:
                                if vox_array[x - 1][y + 1][z] == 0:
                                    a = np.array([1, 1, 0])  #1 
                                    b = np.array([-1, 1, 0])
                                    c = np.array([-1, 1, 1])
